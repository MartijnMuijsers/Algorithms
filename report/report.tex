...
Collapsing an outline of $P$ inwards such that the resulting polygon will contain all points in $P$ implies all points must be inside the polygon defined by this outline. For this purpose, we start with generating the convex hull (the smallest convex set that contains $P$) as our initial set of segments. To compute the convex hull, the Graham scan\cite{convex} is used, an algorithm that finds all vertices of the convex hull ordered along its boundary. After that we define a likelihood function, for which two existing possibilities are listed below:
...
These operations can however give intersecting segments and segments that are extremely long, which mean they are likely not part of the correct solution. To solve these issues, the algorithm will remove all lines that seem inappropriate.
...
The algorithms stated in this paper could be seen to have specific strengths and weaknesses. While designed to give an aesthetically pleasing result, none of the algorithms can fully fulfill the concept of beauty or correctness. It is important to remember that the correctness of the algorithms is always partly subjective, and that any algorithm made for the purpose of a sufficient result in the eyes of most humans will always present solutions that are incorrect in the eyes of some human. That being said, however, it is also true that the presented algorithms give solutions that contain fallacies in the eyes of most humans.
