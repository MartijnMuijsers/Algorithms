...
%TODO Define Node/Point, Segment/Line Segment
...
\begin{itemize}
\item \textbf{Adjacency matrix} - By calculating an adjacency matrix in advance, the algorithm can find nearby points in constant time.
...
After creating these primitives, it becomes relatively straightforward to construct a graph from a set of nodes such that the result can be viewed as an appropriate way of connecting the nodes. The details for this algorithm are elaborated in the next section.
