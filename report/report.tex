...
\begin{abstract}
Curve reconstruction problems  arise  in  many  real-life applications. In this study, while limiting the scope of research to the 2D plane, two formulations of solutions to effectively solve the problem of connecting a set of nodes in an unorganized set in the 2D plane in an aesthetically pleasing manner, and one formulation of a solution for reconstructing a road network from a set of nodes in the 2D plane are proposed.
The first proposed solution reconstructs from a set of nodes a single curve, such that no lines intersect.
The second proposed solution will yield a similar result as the first solution, with the difference that it will output at least two distinct curves.
The third proposed solution attempts to create a road network from nodes. This problem is distinct from the problems just presented, as here the challenge lies in creating intersections, instead of avoiding them.
